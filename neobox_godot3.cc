...
public: // AudioDriverSW
	...
	SpeakerMode get_speaker_mode() const NEOBOX_OVERRIDE {
		return speaker_mode_;
	}
	Array get_device_list() NEOBOX_OVERRIDE {
		Array list;
		list.push_back(default_audio_device);
		return list;
	}
	String get_device() NEOBOX_OVERRIDE {
		return default_audio_device;
	}
	void set_device(String device) NEOBOX_OVERRIDE {
	}
...
public: // OS
...
#if NEOBOX_GODOT_VERSION >= 30100
	int get_current_video_driver() const NEOBOX_OVERRIDE {
	...
	bool _check_internal_feature_support(const String &feature) NEOBOX_OVERRIDE {
		if (video_driver_ == VIDEO_DRIVER_GLES3 && feature == "etc2")
			return true;
		return feature == "mobile" || feature == "etc";
	}
	String get_config_path() const NEOBOX_OVERRIDE {
	...
	String get_data_path() const NEOBOX_OVERRIDE {
	...
	String get_cache_path() const NEOBOX_OVERRIDE {
	...
	Error initialize(const VideoMode &desired, int video_driver, int audio_driver) NEOBOX_OVERRIDE {
		video_mode_ = desired;
	...
	Point2 get_mouse_position() const NEOBOX_OVERRIDE {
		return mouse_pos_;
	}
	...
	void _set_use_vsync(bool enable) NEOBOX_OVERRIDE {
		os_.set_use_vsync_gl(enable);
	}
	void set_icon(const Ref<Image> &icon) NEOBOX_OVERRIDE {
		if (icon.is_null())
			return;
...
#if NEOBOX_GODOT_VERSION >= 30500
	float get_screen_refresh_rate(int screen) const NEOBOX_OVERRIDE {
